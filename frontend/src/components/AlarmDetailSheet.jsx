...
// MAIN COMPONENT
// ════════════════════════════════════════════════════════════════
export default function AlarmDetailSheet({ alarm: initialAlarm, onClose, user, onAck }) {
  ...
  const alreadyAcked = !!initialAlarm?.ack_at;
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '16px 18px 32px' }}>
          ...
          {alreadyAcked && (
            <div style={{
              ...
              background: initialAlarm.kesimpulan === 'valid'
                ? 'rgba(34,197,94,0.08)' : initialAlarm.kesimpulan === 'invalid'
                ? 'rgba(239,68,68,0.08)' : 'rgba(234,179,8,0.08)',
              border: `1px solid ${initialAlarm.kesimpulan === 'valid'
                ? 'rgba(34,197,94,0.3)' : initialAlarm.kesimpulan === 'invalid'
                ? 'rgba(239,68,68,0.3)' : 'rgba(234,179,8,0.3)'}`,
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
                  stroke={initialAlarm.kesimpulan === 'valid' ? '#22c55e' : initialAlarm.kesimpulan === 'invalid' ? '#ef4444' : '#ca8a04'}
                  strokeWidth="2.5">
                  <polyline points="20 6 9 17 4 12" />
                </svg>
                <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text)' }}>Sudah Divalidasi</span>
                <KesimpulanBadge value={initialAlarm.kesimpulan} />
              </div>
              <div style={{ fontSize: 11, color: 'var(--muted)', lineHeight: 1.5 }}>
                <div>Oleh: <strong>{initialAlarm.ack_by}</strong> · {new Date(initialAlarm.ack_at).toLocaleString('id-ID')}</div>
                {initialAlarm.keterangan && (
            ...
            <div style={{ marginBottom: 16 }}>
              {!showForm ? (
                <button
                  ...
                  style={{
                    width: '100%', border: '1px solid #eab308',
                    borderRadius: 10, padding: '10px 0',
                    background: 'rgba(234,179,8,0.1)',
                    color: '#ca8a04', fontWeight: 700, fontSize: 13,
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button
                      type="button"
                      onClick={() => { setShowForm(false); setSaveError(''); setKesimpulan(''); setKeterangan(''); }}
                      style={{ flex: 1, background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, padding: '9px 0', color: 'var(--muted)', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}
                    >
                      Batal
                    </button>
                    <button
                      ...
                      disabled={saving}
                      style={{
                        flex: 2, border: 'none', borderRadius: 8, padding: '9px 0',
